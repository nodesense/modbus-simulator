...
class DeviceBuffer {
    constructor(slaveId) {
        ...
        this.holdingRegisters = Buffer.alloc(1000);
        for (let i = 0; i < 1000; i++) {
            this.holdingRegisters[i] = i % 125; 
        }

        this.inputRegisters = Buffer.alloc(1000);
    }

    readHoldingRegistersInt16(address) {
        return this.holdingRegisters.readInt16BE(address);
    }

    readHoldingRegistersInt8(address) {
        return this.holdingRegisters.readInt8(address);
    }
}
...
class ModbusSlave {
    constructor(port, options) {
        ...
        this.requestFrame  = {
          ...
          this.deviceBuffers = [];
          for (let  i = 1; i <= 247; i++) {
              this.deviceBuffers[i] = null;
              let deviceBuffer = new DeviceBuffer(i);
              this.deviceBuffers[i] = deviceBuffer;
          }

          
          this.responseBuffer = Buffer.alloc(256);
    }
    ...
    open() {
        ...
        this.serialPort.on('readable', (err) => this.readable());
...
const slave = new ModbusSlave("/dev/tty.wchusbserial1410", {
    baudRate:9600,
    databits: 8,
    parity: 'none',
    stopBits: 1,
